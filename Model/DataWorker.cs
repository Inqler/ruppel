...
namespace ManageStaffDBApp.Model
{
    public static class DataWorker
    {
        ...
        //получить всех employe
        public static List<Employe> GetAllEmploye()
        {
            using (ApplicationContext db = new ApplicationContext())
            {
                var result = db.Employes.ToList();
                return result;
            }
        }
        //создать отдел
        ...
        // Create employe
        public static string CreateEmploye(string name, string pass)
        {
            string result = "Уже существует";
            using (ApplicationContext db = new ApplicationContext())
            {
                var checkIsExist = db.Employes
        ...
        //получение employe 
        public static bool CanLogIn(string name, string pass)
        {
            using (ApplicationContext db = new ApplicationContext())
            {
                bool checkIsExist = db.Employes.Any(el => el.Name == name && el.Password == pass);
                return checkIsExist;
            }
        }
